...
a1000 <- as.data.frame(unique(a200[,c('area1000')]))
names(a1000) <- c('area')

output1000 <- d[,c('lat','lon','day','cell1000','ones')]
names(output1000) <- c('lat','lon','period','area','ones')
...
write.table(output1000,"/home/finn/phd/data/20140409/events1000_bd.txt",sep="|",row.names=FALSE,col.names=TRUE,quote=FALSE)
write.table(a1000,"/home/finn/phd/data/20140409/cells1000_b.txt",sep="|",row.names=FALSE,col.names=TRUE,quote=FALSE)

a5000 <- data.frame(unique(a200[,c('area5000')]))
#a5000$areaall <- rep(1,nrow(a5000))
#names(a5000) <- c('area','areaall')
names(a5000) <- c('area')
output5000 <- d[,c('lat','lon','week', 'cell5000','ones')]
names(output5000) <- c('lat','lon','period','area','ones')
...
write.table(output5000,"/home/finn/phd/data/20140409/events5000_b.txt",sep="|",row.names=FALSE,col.names=TRUE,quote=FALSE)
write.table(a5000,"/home/finn/phd/data/20140409/cells5000_b.txt",sep="|",row.names=FALSE,col.names=TRUE,quote=FALSE)

...
```{r}
features = read.table("/home/finn/phd/data/features2.txt", header=FALSE, sep="\t")
# V4 and V5 are day of week and week of month and are categorical variables
features$V4 <- as.factor(features$V4)
features$V5 <- as.factor(features$V5)
features[,6:(ncol(features))] <- scale(features[,6:(ncol(features))])
tmp = features[-c(1,2,4,5)]
lm1 <- lm(V3~.,tmp)

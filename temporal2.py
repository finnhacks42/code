...
def calc_frequency(area,all_dates): #all_dates must be a sorted list, from frist to last, of all the dates to consider (without gaps)
    ...
    by_days_since = {} # maps from number of days since last crime to the number of crimes
    day_since_to_ndays = {} # maps from the number of days since the last crime to the number of days that fall within this bucket
    days_counted = 0
    ...
    for date in all_dates:
        ...
        if last_crime_indx >= 0: # if this is not the first day on which crime occured in this area
            ...
            tmp = by_days_since.get(days_since,0)
            by_days_since[days_since] = tmp + crime_count
            tmp = day_since_to_ndays.get(days_since,0)
            day_since_to_ndays[days_since] = tmp + 1
            
    background = total_crime/float(days_counted) #average number of crimes per day in this area
    #means = {}
    #for days_since,num_crimes in by_days_since.iteritems():
    #    daycount = day_since_to_ndays[day_since]
    #    means[days_since] = num_crimes/float(daycount) - background
    ...
    return [by_days_since,day_since_to_ndays,background] 
...
results = {}
head = ["reportingarea","days_since","num_days","num_crimes","av_num_crimes","av_num_crimes_ad","percent_change","background"]
print "\t".join(head)
for area in area_to_date:
    freqs = calc_frequency(area,all_dates)
    if freqs[2] > 0:
        for days_since, num_crimes in freqs[0].iteritems():
            num_instances = freqs[1][days_since]
            background = freqs[2]
            per_crime_days = num_crimes/float(num_instances)
            per_crime_days_ad = per_crime_days - background
            percent_change = per_crime_days_ad/background
            output = [area,days_since,num_instances,num_crimes,per_crime_days,per_crime_days - background,percent_change,background]
